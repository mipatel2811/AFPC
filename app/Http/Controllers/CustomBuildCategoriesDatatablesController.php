...
class CustomBuildCategoriesDatatablesController extends Controller
{
    ...
    public function getData(Request $request)
    {
        return Datatables::make($this->getForDataTable($request->all()))
            ...
            ->editColumn('image', function ($category) {
                if ($category->image != '') {
                    return "<img src='" . URL::to('/') . "/public/img/custom-build-categories/" . $category->image . "' height='100' width='100'/>";
                } else {
                    return "N/A";
                }
            })
            ...
            ->editColumn('created_at', function ($category) {
                ...
                return "<a href='" . route('custom-build-categories.edit', $category->id) . "' class='btn btn-tool'><i class='fas fa-pen'></i></a>
                <a  href='javascript:;' class='btn btn-tool delete_" . $category->id . "' data-url='" . route('custom-build-categories.destroy', $category->id) . "'  onclick='deleteRecorded(" . $category->id . ")'><i class='fas fa-trash'></i></a>";
            })
            ->make(true);
    }
    public function getForDataTable($input)
    {
        ...
        $dataTableQuery = CustomBuildCategories::orderBy('id', 'desc');
